vscode.activate = function (e) {
	console.log('activating cursor retrieval extension', 'isDev:', e.isDevelopment), IndexingRetrievalLogger.init(), l.CursorGitGraphLogger.init(), e.isDevelopment && l.CursorDebugLogger.init(), (0, d.runWorkspaceStateMigrations)(e)
...
class FastIndexer {
	constructor(e) {
		this.context = e, this.disposables = [], this.status = new m.LocalIndexingStatus, this.currentIndexingJobs = new m.CurrentIndexingJobs, this.highLevelDescriptionGetter = new f.HighLevelDescriptionGetter(e), this.getInitialValues = (async () => {
			try {
				...
				IndexingRetrievalLogger.error('failed to create the client', e)
			}
		})()
		const t = {
		...
		this.disposables.push(r), this.status.init(), this.disposables.push(a.cursor.onDidChangeCursorAuthToken((e => {
			this.accessToken = e, this.createRepositoryClient().catch(IndexingRetrievalLogger.error).then((() => {
				this.createRepoIndexWatcherIfShouldIndex({shouldRecreateIfExists: !0})
			}))
		}))), this.disposables.push(a.cursor.onDidChangeCursorCreds((e => {
			this.backendUrl = e.repoBackendUrl, this.createRepositoryClient().catch(IndexingRetrievalLogger.error).then((() => {
				this.createRepoIndexWatcherIfShouldIndex({shouldRecreateIfExists: !0})
			}))
			...
			}), void IndexingRetrievalLogger.error(e.error ?? 'Unknown error')
			const t = e.info
			IndexingRetrievalLogger.info('Setting indexing intent to should-index'), this.setIndexingIntent(t, A.IndexingIntent.ShouldIndex), await this.createRepoIndexWatcherIfShouldIndex({shouldRecreateIfExists: !1})
	...
	async createRepoIndexWatcherIfShouldIndex(e) {
		try {
			...
			if (await this.getInitialValues, void 0 === this.accessToken) return void IndexingRetrievalLogger.error('accessToken is undefined. User is not logged in. We shouldn\'t do any indexing.')
			...
			if (!(s === A.IndexingIntent.FallBackToDefault ? n : s === IndexingIntent.ShouldIndex)) return IndexingRetrievalLogger.info('Not indexing because user does not want to index this workspace.'), void this.status.set({case: 'not-indexed'})
	...
	async createRepositoryClient() {
		void 0 !== this.repoClient && this.repoClient.dispose(), void 0 !== this.accessToken && void 0 !== this.backendUrl ? this.repoClient = new RepoClientMultiplexer(this.accessToken, this.backendUrl) : this.repoClient = void 0
	}
	...
	getWorkspaceRootInfoWithProperError() {
		...
		return IndexingRetrievalLogger.setUser(r), IndexingRetrievalLogger.setRepo(o.repoName), {
			info: i,
			error: void 0
		}
	}
...
class RepoIndexWatcher {
	constructor(e, t, r, n, s, o) {
		this.repoInfo = e, this.repoClient = t, this.currentIndexingJobs = r, this.status = n, this.indexingIntent = s, this.context = o, this.disposableIntervals = [], this.isDisposed = !1, this.currentIndexingJob = void 0, IndexingRetrievalLogger.debug('Constructing repo index watch.'), this.disposableIntervals.push(setInterval((async () => {
			this.doUpdate()
		}), 6e5)), this.doUpdate()
	}
	...
	async doUpdate() {
		try {
			...
			IndexingRetrievalLogger.debug('Doing repo index watch update.'), void 0 !== this.currentIndexingJob && this.currentIndexingJob.dispose(), (0, y.isPathEncryptorAvailable)() ? this.repoInfo.setUseLegacyRepoName(!1) : this.repoInfo.setUseLegacyRepoName(!0)
			const e = await a.commands.executeCommand(w.ConfigServiceActions.GetCachedServerConfig)
			let t
			t = void 0 === e || null == e || void 0 === e.indexingConfig ? R : e.indexingConfig,
				this.currentIndexingJob = new IndexingJob(this.repoInfo, this.repoClient, this.currentIndexingJobs, this.status, this.indexingIntent, this.context, t), this.currentIndexingJob.result.then((() => {
				IndexingRetrievalLogger.info('Indexing job successfully done or aborted.')
			})).catch((e => {
				let t = e ? e.toString() : 'Unknown error'
				IndexingRetrievalLogger.error('Indexing job failed.', e), this.status.set({
					case: 'error',
			...
			let t = e ? e.toString() : 'Unknown error'
			IndexingRetrievalLogger.error('Unhandled error in doUpdate', e), this.status.set({
				case: 'error',
				error: `Unknown error: ${t}`
			})
		}
	...
	dispose() {
		IndexingRetrievalLogger.debug('Disposing repo index watch.'), this.currentIndexingJob?.dispose(), this.isDisposed = !0, this.disposableIntervals.forEach((e => {
			clearInterval(e)
		}))
	}
}
...
class IndexingJob {
	constructor(repoInfo, repoClient, currentIndexingJobs, status, indexingIntent, context, config) {
		...
		this.highLevelDescriptionJob = new HighLevelDescriptionJob(this.repoClient, this.merkleClient, this.context)
...
class HighLevelDescriptionJob {
    ...
    async compute() {
        try {
            ...
            IndexingRetrievalLogger.debug(`Computing high level folder description for ${e.length} files at ${r}`)
            try {
                ...
                IndexingRetrievalLogger.warn('Error reading high level folder description. Ignoring.', e)
            }
            ...
            IndexingRetrievalLogger.warn('Error computing high level folder description. Ignoring.', e)
        }
    }
    ...
    async computeFull(e) {
        try {
            ...
            IndexingRetrievalLogger.warn('Error computing full high level folder description. Ignoring.', e)
        }
    }
    ...
    async syncMerkleSubtreeWithRetry(e, t) {
        ...
        for (; r < 6e4;) {
            ...
            try {
                ...
                await Promise.race([n, s]), r *= 2, IndexingRetrievalLogger.warn(`Retrying sync merkle subtree with timeout ${r}. Error: `, e)
            }
        }
    ...
    async deleteFastUpdateFileWithRetry(e, t) {
        ...
        for (; r < 24e4;) {
            ...
            try {
                ...
                await Promise.race([n, s]), r *= 2, IndexingRetrievalLogger.warn(`Retrying fast update file (delete) with timeout ${r}. Error: `, e)
            }
        }
    ...
    async handshakeWithRetry(e, t) {
        ...
        for (; r < 12e4;) {
            ...
            try {
                ...
                IndexingRetrievalLogger.info('Handshake start')
                const s = await this.get().fastRepoInitHandshake(e, {
                    signal: AbortSignal.any([t, this.networkChangeAbortController.signal]),
                    timeoutMs: r
                }), o = performance.now()
                if (IndexingRetrievalLogger.info('Handshake timing: ' + (o - n)), s.status === p.FastRepoInitHandshakeResponse_Status.FAILURE) throw new Error('FastRepoInitHandshakeResponse_Status.FAILURE in handshakeWithRetry')
                ...
                await Promise.race([n, s]), r *= 2, IndexingRetrievalLogger.warn(`Retrying handshake with timeout ${r}. Error: `, e)
            }
        }
    ...
    async ensureIndexCreatedWithRetry(e, t) {
        ...
        for (; r < n;) {
            ...
            try {
                ...
                await Promise.race([s, o]), r *= 2, IndexingRetrievalLogger.warn(`Retrying ensure index created with timeout ${r}. Error: `, e)
            }
        }
